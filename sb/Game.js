...
SeaBombs.Game.prototype = {
    ...
    create: function() {
        ...
        this.poosh =  this.add.audio('poosh');
    ...
    moveTargetUp: function() {
        if (!this.gameover) {
            if (this.target && this.target.y > 150) {
                this.target.y -= 5;
            }
        }
    },   
    ...
    fireShot: function() {
        if (!this.gameover) {
            if (this.ship.exists) {
                // there can only be x shots on the screen at once:
                if (this.time.now > this.nextfire && this.shots.countDead() > 0 && this.shots.countLiving() < 6) {
    ...
    startStopPing: function() {
        if (this.enemy1.exists === true || this.enemy2.exists === true || this.enemy3.exists === true) {
            this.pingTimer = this.time.now + 100;
            if (this.ping.isPlaying === false) {
                this.ping.loopFull(0.1);
                console.log('>>>>>>>>>>>>>>looping ping');
            }
        } else {
            console.log('No enemies alive. stopping ping');
            this.ping.stop();
            this.pingTimer = null;
        }
    },
    ...
    update: function() {
        ...
        if (this.gameover === false) {
            ...
            if (this.pingTimer < this.time.now) {
                    this.startStopPing();
            }
