    ...
    create: function () {
      ...
      if (!this.game.device.localStorage) {
        ...
        var ns = localStorage.getItem('newestScore') ? localStorage.getItem('newestScore') : 0;
    		...
    		this.highscoretitle = this.add.bitmapText((this.world.width/2)-300,(this.world.height/2)-200, 'eightbitwonder', 'Previous High Score: ' + phs, 20);
    		this.yourscoretitle = this.add.bitmapText((this.world.width/2)-300,(this.world.height/2)-160, 'eightbitwonder', 'Your Score: ' + ns, 20);
        phs = parseInt(phs, 10);
        ns = parseInt(ns, 10);
        if (ns > phs) {
                console.log('HighScores: replacing high score with newest score');
                localStorage.setItem('highScore', ns);
                this.newhighmessage = this.add.bitmapText((this.world.width/2)-300,(this.world.height/2)-120, 'eightbitwonder', 'You got the new high score!!!', 15);
    		} else { 
            //alert('ns ' + ns + ' is less than phs ' + phs + ' so not a new high score');
            this.newhighmessage = this.add.bitmapText((this.world.width/2)-300,(this.world.height/2)-120, 'eightbitwonder', 'Try again...', 15);
    		}
      }
  		
  		this.overmessage = this.add.bitmapText((this.world.width/2)-300,(this.world.height/2)-80, 'eightbitwonder', 'Click or press Space to Play Again', 20);
